#!/usr/bin/env bash
# Rust stdlib HTTP server (std::net::TcpListener → libc → nfl).
...
run_client() {
    ...
    printf '%s\n' "$out" >> "$CLIENT_LOG"
    grep -q '^nfl-rust-ok' <<< "$out" || return 1
    return 0
}
...
run_test() {
    ...
    (
        ...
        exec /opt/rustapp/server
    ) >> "$DAEMON_LOG" 2>&1 &
    daemon_pid=$!

    if ! wait_for_listener; then
        echo "[rust-http] FAIL: daemon did not start" >&2
        tail -n 40 "$DAEMON_LOG" >&2 || true
        tail -n 40 "$BRIDGE_LOG" >&2 || true
        return 1
    fi
    if run_client; then echo "[rust-http] PASS"; return 0; fi
    echo "[rust-http] FAIL: client failed" >&2
