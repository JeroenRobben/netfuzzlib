...
This framework emulates Linux network I/O in userspace and routes network socket related calls like `accept()`/`recv()`/`send()`/`poll()` to the callbacks in [`include/netfuzzlib/callbacks.h`](include/netfuzzlib/callbacks.h). You can use this for efficient [fuzzing over network I/O](examples/afl), or any other case where you don't want network I/O to go through the kernel.
...
To use netfuzzlib, you need to implement the callbacks in `callbacks.h` and link both your code and the core framework (libnfl.so/a) with the target, e.g., by using `LD_PRELOAD`. See [`examples/hello-world`](examples/hello-world) to get started.
...
The framework is **not multi-process aware**: after fork(), each process has its own copy of the network environment, so modelled sockets can't be used for IPC between processes, and calling exit() from a callback only terminates the current process.
