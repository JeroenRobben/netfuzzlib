...
int nfl_set_ipv4_default_gateway(const char *gateway_addr_text, const unsigned int device_index) {
    nfl_l2_iface_t *l2_iface = get_l2_iface_by_index(device_index);
    if (!l2_iface) {
        nfl_log("Could not find l2_iface with index: %d, not setting ipv4 default gateway", device_index);
        return -1;
    }
    ipv4_default_gateway *ipv4_gateway = calloc(1, sizeof(ipv4_default_gateway));
    if (!ipv4_gateway) {
        errno = ENOBUFS;
        nfl_log("Out of heap space, not setting ipv4 default gateway", device_index);
        return -1;
    }
    const int ret = inet_pton(AF_INET, gateway_addr_text, &ipv4_gateway->gateway_addr);
    if (ret != 1) {
        nfl_log("Could not parse ipv4 address: %s, not setting ipv4 default gateway", gateway_addr_text);
    }

    for (nfl_l3_iface_t *l3_iface = l2_iface->l3_interfaces; l3_iface; l3_iface = l3_iface->next) {
    ...
    if (!ipv4_gateway->interface) {
        nfl_log("Default gateway address %s was not within subnet of ipv4 interface of l2_iface %s with index %d, not setting ipv4 default gateway",
    ...
    nfl_log("Default IPv4 gateway set to %s | l2_iface %s", gateway_addr_text, ipv4_gateway->interface->parent_l2_iface->name);
    return 0;
}

int nfl_set_ipv6_default_gateway(const char *gateway_addr_text, const unsigned int device_index) {
    nfl_l2_iface_t *l2_iface = get_l2_iface_by_index(device_index);
    if (!l2_iface) {
        nfl_log("Could not find l2_iface with index: %d, not setting ipv6 default gateway", device_index);
        return -1;
    }
    ipv6_default_gateway *ipv6_gateway = calloc(1, sizeof(ipv6_default_gateway));
    if (!ipv6_gateway) {
        errno = ENOBUFS;
        nfl_log("Out of heap space, not setting ipv6 default gateway", device_index);
        return -1;
    }
    const int ret = inet_pton(AF_INET6, gateway_addr_text, &ipv6_gateway->gateway_addr);
    if (ret != 1) {
        nfl_log("Could not parse ipv6 address: %s, not setting ipv6 default gateway", gateway_addr_text);
    }
    ...
    if (!ipv6_gateway->interface) {
        nfl_log("Default gateway address %s was not within subnet of ipv6 interface of l2_iface %s with index %d, not setting ipv4 default gateway",
                     gateway_addr_text, l2_iface->name, device_index);
        free(ipv6_gateway);
        return -1;
    }
    ...
    nfl_log("Default IPv6 gateway set to %s | l2_iface %s", gateway_addr_text, ipv6_gateway->interface->parent_l2_iface->name);
    return 0;
}
...
nfl_l3_iface_t *routing_table_lookup(const nfl_addr_t *dest_addr) {
    ...
    case AF_INET6: {
        if (IN6_IS_ADDR_V4MAPPED(&dest_addr->s6.sin6_addr)) {
            const struct in_addr v4 = { .s_addr = dest_addr->s6.sin6_addr.s6_addr32[3] };
            result = routing_table_lookup_ipv4(&v4);
        } else {
    ...
    default:
        nfl_die(1, "Invalid address family in routing_table_lookup");
    }
    if (!result) {
        nfl_log("Routing table lookup failed: could not find device that can reach %s", sockaddr_to_str_static_alloc(dest_addr));
    }
    return result;
}
